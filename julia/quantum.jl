...
# ╔═╡ 1442f792-2d52-11ee-02be-37ae48cc2f04
begin
...
function twist(arr, f = tuple; n = n)
    f.((reshape(arr, Int.(ones(i - 1))..., length(arr)) for i = 1:n)...)
end

# ╔═╡ 45460016-0cc6-4f47-a087-d041058e4f43
begin
    L1 = pid^2 * [k^2 for k in fftfreq(N, N)]
    L = twist(L1, +)
    laplacian(q) = fft(q) .* L |> ifft
end

# ╔═╡ 26962212-c379-4552-b997-19f788ca23d2
begin
    xs = range(0; length = N, step = h)
    V =
        V0 * twist(
            xs,
            (x...) ->
    ...
    @time @gif for t = 0:1/fps:T
        @chain sol(t) abs2.() plot(
            ...
            [
                begin
                    cmp = tuple((1:i-1)..., (i+1:n)...)
                    dropdims(sum(_, dims = cmp), dims = cmp) * h^(n - 1)
                end for i = 1:n
            ],
            ylims = (0, 2),
            legend = false,
...
git-tree-sha1 = "3ea1eb1ce9a35e1d604a3d99008f3570537ff6f4"
